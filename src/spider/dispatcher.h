		    ...
		    [&, this](auto&& p) {
			    ...
			    if constexpr (std::is_same_v<P, path_parameter> && !ConvertibleFromBoostJson<T>)
			    {
				    ...
				    if (m.matched)
				    {
					    ...
					    {
						    throw std::invalid_argument{ fmt::format("'{}': could not convert {} to {}: {}",
							                                         p.name,
...
struct handler : public handler_base
{
	...
	virtual message_generator call(request&& req, url_view&& url, string_view path, const svmatch& match)
	{
		...
		{
			SPIDER_LOG(err, "Could not collect parameters: {}", e.what());
